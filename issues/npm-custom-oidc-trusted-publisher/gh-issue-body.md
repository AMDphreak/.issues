...
Trusted Publishing on npm only accepts OIDC tokens from a fixed set of issuers (GitHub Actions, GitLab CI, CircleCI). Third-party publishing orchestration services, custom CI, or other OIDC-capable IdPs cannot be used—their tokens are rejected. The only workaround is long-lived API tokens, which undermines the security and UX benefits of OIDC.
...
Support **custom OIDC trusted publishers**: allow users to register a trusted publisher with an **Issuer URL** and **Subject identifier** (or equivalent claim). Configuration remains one-time and interactive in npm; the registry validates the token's issuer and subject against the configured values. Optionally, an allowlist of issuer hostnames could be used to restrict which custom IdPs are accepted.

## Why it matters

- More teams can use short-lived OIDC tokens instead of long-lived npm tokens.
- Centralized or third-party publishing tools can support npm without handling secrets.
- Parity with other registries (PyPI, Crates.io) that are considering generic/custom OIDC.

**Target:** npm registry backend (issuer validation) and/or npm/cli (if custom-issuer config is exposed via API).  
**Related:** [Trusted publishing for npm packages](https://docs.npmjs.com/trusted-publishers).
