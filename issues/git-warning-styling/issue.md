...
When Git/SSH outputs the informational message "warning: permanently added 'gitlab.com' to the list of known hosts", the editor displays it with error-level styling (red error icon with X), making it appear as a critical error when it's actually just an informational message.
...
This issue likely stems from the editor parsing Git output and mapping the "warning" keyword to error-level styling. The message originates from Git/SSH, not the editor, but the editor's UI is applying incorrect styling.

## Environment

- Editor version: [current version]
- OS: Windows 11
