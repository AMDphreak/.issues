...
- **ARP path:** After ARP correlation finds the installed package, compare `Utility::Version` of the correlated **installed** version to the **manifest** package version. If `installed < expected`, fail with `APPINSTALLER_CLI_ERROR_UPGRADE_INSTALLED_VERSION_MISMATCH` (`0x8A15008F`).
